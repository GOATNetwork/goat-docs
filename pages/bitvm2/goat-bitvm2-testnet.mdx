...
Combined with GOAT Network’s decentralized sequencer and zkMIPS, GOAT BitVM2 enables GOAT Network to be the first native Bitcoin zkRollup with real yield. See more at [GOAT BitVM2 while paper](https://www.goat.network/bitvm2-whitepaper).
...
### Path 1
Only Operators participating in Peg-in can use this path to bridge out a specific amount of BTC.
Simply connect your EVM wallet.

   <p align="center"> <img src="/img/BitVM2/pic9.png" width="90%"/> </p>

Select the amount you wish to bridge out.

   <p align="center"> <img src="/img/BitVM2/pic11.png" width="90%"/> </p>

Click [Approve] — your wallet will prompt an approval window. Please approve the maximum amount you intend to transact.

   <p align="center"> <img src="/img/BitVM2/pic12.png" width="90%"/> </p>

After the approval is successful, return to the interface and click [Submit]. Your wallet will prompt a transaction confirmation window. Confirm the transaction and pay the gas fee.

   <p align="center"> <img src="/img/BitVM2/pic13.png" width="90%"/> </p>

In Step 2, this indicates you have successfully kicked off the process. You can click [BitVM2 Graph] to track the bridge-out progress.

   <p align="center"> <img src="/img/BitVM2/pic14.png" width="90%"/> </p>

Once the bridge-out is successful, the funds will appear in your receiving wallet.
...
Bridge out BTC from GOAT network to Bitcoin mainnet through GOAT Bridge；
In GOAT Bridge, select the Bitcoin chain and use the bridge out function.

   <p align="center"> <img src="/img/BitVM2/pic18.png" width="90%"/> </p>

Detailed steps for bridging out can be found in the [GOAT Bridge User Guide](/users/bridge).
...
| Challenger|  Anyone can be a challenger: 1) Verify the valid of the reimbursement from operators offchain; 2) Submit the challenge transaction on Bitcoin to force the kick off to unhappy path; 3) Once the kickoff is on the unhappy path, and the operator unveils all the execution trace(Circuit F below), verify finds the flow in the execution trace, and can spend the UTXO from Assert transaction, and stop the operator to continue the reimbursement. | 1/∞ |
...
BITVM_SECRET:  your HEXHex-format private key, which is used to submit a challenge to the operator's reimbursement in BitVM2 graph.  
...
```shell
bitvm2-noded --bootnodes ${bootnodes} -d --rpc-addr localhost:8081 --p2p-port 8443 --db-path ${path-to-workspace}/bitvm2-node.db2 
```
