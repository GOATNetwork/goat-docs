---
title: GOAT BitVM2 Testnet3 Beta
lang: en-US
---

import { Callout, Tabs, Tab } from 'nextra/components'

# **GOAT BitVM2 Testnet3 Beta**

## Introduction

<Callout type="info">
**GOAT BitVM2 Testnet3 Beta was born to enable efficient interoperability between the Bitcoin and the GOAT Network.**
</Callout>
...
GOAT BitVM2 extends BitVM2 into a standard Bitcoin zkRollup by solving key limitations such as operator double-spending, inefficient reimbursement, and lack of incentives mechanism. GOAT BitVM2 can allow any operator to finish the reimbursement in 1 day with the powder of Ziren(formally zkMIPS) and multi-round randomized challenge mechanism, and prevent the operator’s double spend issues by verifying the Rollup chain’s consensus lightclient. 

Combined with GOAT Network’s decentralized sequencer and Ziren, GOAT BitVM2 enables GOAT Network to be the first native Bitcoin zkRollup with real yield. See more at [GOAT BitVM2 White Paper](https://www.goat.network/bitvm2-whitepaper).
...
The real-time proving is now live on BitVM2 Testnet3 Beta. This major milestone is the first implementation of a BitVM2 Bridge capable of generating and verifying withdrawal proofs in sync with block production - without delay or backlog. 

The system is powered end-to-end by Ziren, ZKM’s production-grade zkVM and proving stack, delivering the throughput, latency, and compression required to make peg-outs on Bitcoin usable in practice.


### Key features

* Instant kickoff availability powered by Ziren: Achieved a block proof average of 2.6s, aggregation proof in 2.7s, and Groth16 proof in ~10.38s.

* Comprehensive proof visualization: Introduced a dedicated proof UI that displays the full proof generation pipeline and detailed validity proofs for each peg-out.

* Capital efficiency for operators: Without real-time proof generation, delays accumulate block by block. This can force operators to wait days or weeks to withdraw.

### Proof UI 

Entrypoint: https://bitvm2.testnet3.goat.network/proof

* Track live proof generation:

   <p align="center"> <img src="/img/BitVM2/proof-ui-1.png" width="90%"/> </p>

* Click on the pipeline to inspect detailed proof stages:

   <p align="center"> <img src="/img/BitVM2/proof-ui-2.png" width="90%"/> </p>

Note: A single proof takes ~13s, but with five provers running in parallel, average time per proof drops below 3s.


* Dashboard: Displays BitVM2 transaction graphs and proof records:

   <p align="center"> <img src="/img/BitVM2/proof-ui-3.png" width="90%"/> </p>

* Click a proof: View the Groth16 proof used for corresponding reimbursement:

   <p align="center"> <img src="/img/BitVM2/proof-ui-4.png" width="90%"/> </p>


## Join GOAT BitVM2 Testnet3 Beta
...
Use the docker image [bitvm2-node](https://github.com/goatnetwork/bitvm2-node/pkgs/container/bitvm2-node).
...
```
docker run -d -v ${data to path}/:/var/data -v $PWD/.env:/app/.env --env-file .env -p 9100:9100 -p 8443:8443 ghcr.io/goatnetwork/bitvm2-node:${version}
```
