...
A separate relayer network handles GOAT’s Bridge operations. See [GOAT BitVM Bridge](/introduction/bridge) for more details.
...
Validators join the network by staking tokens through the Locking smart contract on the execution layer. For more details, refer to [Validator Guide](/builders/sequencers/validator).
