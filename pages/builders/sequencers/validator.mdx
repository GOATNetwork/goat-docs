...
description: 'Complete guide for setting up and running a sequencer node (2 BTC stake) on GOAT Testnet3'
---
...
<Callout type="info">
  This guide is for running a Sequencer Node, which requires a 2 BTC lock. This is similar to Bitcoin mining, where multiple nodes can participate. For running a Sequencer Node **Operator** position (2 BTC, like Goat.Network, Amber, Metis), see [Running a Sequencer Operator](/builders/sequencers/running).
</Callout>
...
Replace `0x...abc` with your Bitcoin private key. The owner is the node administrator's BTC address responsible for locking, redemption, and other operations.
